...
export function SidebarItem({ icon, text, active, alert, location }: SidebarItemProps): JSX.Element {
    ...
    return (
        <li
            ...
            className={`relative flex items-center py-2 px-3 my-1 font-medium rounded cursor-pointer transition-colors group 
        ${active ? 'bg-blue-800 hover:bg-blue-700 text-blue-50' : 'hover:bg-neutral-100 text-neutral-900 hover:text-neutral-800'}`}
        >
            {icon}
            <span className={`overflow-hidden transition-all ${expanded ? 'w-52 ml-3' : 'w-0'}`}>{text}</span>
            ...
            {!expanded && (
                <div
                    className={`absolute left-full rounded px-2 py-1 ml-6 bg-orange-500 z-50 text-orange-50 text-sm invisible opacity-20
                    -translate-x-3 transition-all group-hover:visible group-hover:opacity-100 group-hover:translate-x-0`}
                >
                    {text}
                </div>
